...
To create a micro-service, a \texttt{yaml} file must be submitted defining the how GAE should respond to requests and giving details of the runtime of your app instance. It also states the command to start up the server, for which we used Gunicorn\cite{gunicorn} as our Web Server Gateway Interface to serve up the web interface. Our folder structure also included a template directory for serving up dynamic pages and a static directory for the static content like style sheets. We also uploaded a \texttt{requirements.txt} file to tell the container build script to pull in the appropriate libraries.
