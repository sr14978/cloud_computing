...
Using containers means that the initial system set up can
take much longer than just stating up the server code as an docker image has to be built, but provides a clean way to
manage the running instances and later creation of clone instances is just as
fast.
% SAM: This seems very out of place - should it just be removed? 
%In the App Engine you cannot use the instance's filesystem to store data
